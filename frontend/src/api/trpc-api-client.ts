 ...
 */
import { vanillaClient as trpcClient } from './trpc-vanilla-client';
import { setAuthToken, clearAuthToken, apiHandler } from '@/utils/trpc';
...
export const auth = {
  ...
  login: async (email: string, password: string) => {
    ...
    if (result.data && typeof result.data === 'object' && 'token' in result.data) {
      const tokenData = result.data as { token: string };
